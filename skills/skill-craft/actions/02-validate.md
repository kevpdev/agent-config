...
2. **Lint R5 — frontmatter.** `SKILL.md` porte un frontmatter avec `name` (égal au dossier) et `description`. Absence de l'un ou l'autre = `FAIL` dur : sans `description`, le harness déclenche mal le skill. Le frontmatter doit aussi **parser en YAML strict** (`python3 -c "import yaml,sys;yaml.safe_load(...)"`), pas seulement porter les bonnes clés : un `:` suivi d'un espace dans un scalaire non quoté passe sous Claude Code et casse sous la spec. *Pourquoi ce contrôle en plus des clés :* un lint qui ne regarde que les clés ne voit pas la syntaxe qui les porte.
   - **Lint R13 — mode d'invocation.** Croiser deux signaux. Le skill **exécute-t-il** un effet de bord (commit, push, écriture versionnée, suppression, envoi réseau) ? Porte-t-il `disable-model-invocation: true` ? Effet de bord sans le champ = `FAIL` : le skill part au flair sur une action irréversible. Champ présent avec une liste de phrases déclencheuses dans la `description` = `FAIL` aussi : elle promet un déclenchement qui n'existe plus.
     - **Ne pas compter les mots-clés, lire ce que le skill fait.** Un skill de revue cite « commit » et « push » pour décrire ce qu'il relit, sans rien exécuter. Mesuré le 2026-08-11 : un comptage d'occurrences refusait `mr-review`, qui se déclare pourtant « en lecture seule » dès sa description. *Pourquoi c'est le défaut à éviter en premier :* un lint qui refuse du travail valide finit désactivé, et on perd aussi les refus justes.

...
5. **Contrôler le corpus d'évals.** Vérifier que `evals/eval.json` parse, qu'il porte au moins un cas positif, et un cas `expect_trigger: false` par frère vivant cité en clause NE PAS. Un fichier d'évals cassé rend le même « rien à signaler » qu'un fichier absent.

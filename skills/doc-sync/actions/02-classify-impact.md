...
1. **Classer.** Pour chaque fichier du scope, déterminer l'impact, le régime, puis la surface cible. Le régime commande le traitement aval (reflet → réécrit, décision → signalé).
   ...
   **En `--audit`, sauter cette table.** Elle classe du **code** par son chemin, et un fichier de mémoire ne matche aucun de ses patterns. Classer alors chaque fichier du scope directement par la **table des régimes du `SKILL.md`**, que l'étape 3 désigne déjà comme la source unique. La cible doc **est** le fichier lui-même, il n'y a pas de cible à déduire.

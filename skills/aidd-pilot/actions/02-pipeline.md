...
2. **Filtrer** :
   - **Triviale** (un port, un nom de paramètre, une signature, un header) → l'`executor` **s'arrête quand même**, on ne peut pas l'en empêcher : `02-implement/actions/02-execute.md` étape 4 impose *« **any** mismatch with the plan, **trivial or substantive**, stop and report `replan needed` »*. Donc corriger la **phase courante seule**, ne toucher à **aucune** phase aval, et relancer un `executor` neuf. Le correctif atterrit dans le commit de la phase courante. **Pas de replan.**
     - *Pourquoi le formuler ainsi :* une consigne « l'`executor` corrige sans s'arrêter » est inapplicable — l'agent piloté ne sait pas le faire, et une règle qu'aucun exécutant ne peut honorer se lit comme un bug du skill. La frontière utile n'est pas « arrêt ou pas » mais **jusqu'où va la réécriture** : phase courante (trivial) vs phases aval (substantiel). C'est aussi ce qui la rend observable.

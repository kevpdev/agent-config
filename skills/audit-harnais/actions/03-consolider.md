...
2. **Rédiger.** Le rapport selon les invariants de la grille : commit de la grille en en-tête (avec son état de travail), contrat et exclusions, puis ces sections aux intitulés imposés — `## Contrat`, `## Cascade par instruction` (tableau instruction × critère de sortie × action × base), `## Captures hors grille`, `## À réviser entre deux audits`. *Pourquoi des intitulés imposés : c'est le seul signal mesurable qu'un rapport vient de ce protocole.*
...
- L'en-tête cite le commit de la grille ; les quatre intitulés imposés sont présents (grep).
